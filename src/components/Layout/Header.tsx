...
const Header: FC = () => {
  ...
  return (
    <div className="dark:bg-black bg-white bg-opacity-80 rounded-[1000px] flex px-2 py-2 border border-primary/50 container items-center relative top-5">
      ...
      <nav className="ml-10 text-lg">
        <ul className="flex gap-10 items-center justify-between">
          {menuData.map((item) => (
            ...
            >
              <Link to={item.href} className="p-2 py-6">
                {item.name}
              </Link>
              <AnimatePresence>
